...
delimiter $$
CREATE DEFINER=`Sup_User_SPESA`@`%` PROCEDURE `tools`.`gen_data_script`(IN SchemeName VARCHAR(64), in tableName VARCHAR(64), in EngineName varchar(64))  ExitSub:BEGIN
...
SELECT
	CONCAT( 'mysqldump -P 3306 -hlocalhost -udba -pdba --no-create-db --no-create-info --column-statistics=0 --skip-triggers --extended-insert ',
